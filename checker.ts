...
function problem2() {
  return prisma.$queryRaw`
    SELECT   e.sin AS sin,
             ...
             e.salary AS salary,
             (mgr.salary - e.salary) AS "Salary Diff"
    FROM     Employee  e
...
function problem3() {
  return prisma.$queryRaw`
    SELECT  c1.firstName as firstName, c1.lastName as lastName, c1.income as income
    FROM    Customer            c1
    LEFT JOIN Customer          c2  ON c2.lastName = 'Butler'
    GROUP   BY c1.customerID
    HAVING  c1.income >= 2 * MAX(c2.income)
    ORDER   BY c1.lastName ASC, c1.firstName ASC
    LIMIT   10;
  `
}
...
function problem4() {
  return prisma.$queryRaw`
    ...
    JOIN   Owns       oAll ON oAll.customerID = c.customerID
    JOIN   Account    aAll ON aAll.accNumber  = oAll.accNumber
    WHERE  c.income  > 80000
    ORDER  BY c.customerID ASC, aAll.accNumber ASC
    LIMIT  10;
  `
}
...
function problem7() {
  return prisma.$queryRaw`
    ...
    JOIN   Branch          bNY     ON bNY.branchNumber = aNY.branchNumber AND bNY.branchName  = 'New York'
    LEFT   JOIN Owns       oAll       ON oAll.customerID = c.customerID
    LEFT   JOIN Owns       oCo        ON oCo.accNumber  = oAll.accNumber
    LEFT   JOIN Owns       oCoLon     ON oCoLon.customerID = oCo.customerID
    LEFT   JOIN Account    aCoLon     ON aCoLon.accNumber  = oCoLon.accNumber
    LEFT   JOIN Branch     bCoLon     ON bCoLon.branchNumber = aCoLon.branchNumber AND bCoLon.branchName  = 'London'
    GROUP  BY c.customerID HAVING COUNT(bCoLon.branchName) = 0
...
function problem10() {
  return prisma.$queryRaw`
    ...
    WHERE   (c.income > 5000
              OR (c.firstName = 'Helen' AND c.lastName = 'Morgan'))
    GROUP   BY c.customerID
    HAVING  COUNT(DISTINCT CASE
               WHEN aC.branchNumber = aH.branchNumber
               THEN aC.branchNumber END)
         =  COUNT(DISTINCT aH.branchNumber)
...
function problem11() {
  return prisma.$queryRaw`
    ...
    LEFT    JOIN Employee e2
                 ON e2.branchNumber = e1.branchNumber
